...
a) Mit dem program:
   ...
   ![Prozessbaum](./imgs/a2-process-tree.png){}
...
* `l` = Argumente werden als Liste übergeben (z. B. `execl`)
* `v` = Argumente als Array (vector) (z. B. `execv`)
* `p` = Aktiviert die Suche über die Umgebungsvariable `PATH` (z. B. `execvp`)
* `e` = Übergabe einer eigenen Umgebung über `envp[]` möglich (z. B. `execle`, `execvpe`)
* Kein `p` = Es muss ein vollständiger Pfad zur ausführbaren Datei angegeben werden
* Kein `e` = Die aktuelle Umgebung des Prozesses wird verwendet
* `fexecve` = Statt eines Pfads wird ein offener File Descriptor verwendet; keine `PATH`-Suche; besonders sicher gegen Race Conditions (z. B. bei temporären Dateien oder in sicherheitskritischen Kontexten)

Wann welche Variante:

| Variante  | Typischer Einsatzzweck                                                                                                            |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------: |
| `execl`   | Wenn der Pfad bekannt ist und Argumente direkt im Code angegeben werden                                                           |
| `execle`  | Wie `execl`, aber zusätzlich mit expliziter Umgebung                                                                              |
| `execlp`  | Wie `execl`, aber mit automatischer PATH-Suche                                                                                    |
| `execv`   | Wenn Argumente in einem Array vorliegen (z. B. aus `main`)                                                                        |
| `execvp`  | Wie `execv`, aber mit PATH-Suche (z. B. für einfache Shells)                                                                      |
| `execvpe` | Wie `execvp`, aber mit eigener Umgebung (nicht POSIX, GNU-Erweiterung)                                                            |
| `execve`  | Low-Level-Version mit voller Kontrolle über Pfad, Argumente und Umgebung                                                          |
| `fexecve` | Wenn die Datei bereits sicher geöffnet wurde, z. B. mit `openat()`; nützlich in Sandboxen oder zur Vermeidung von Race Conditions |
